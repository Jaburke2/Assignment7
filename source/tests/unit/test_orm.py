...
class TestORM(unittest.TestCase):
    ...
    def setUp(self):
        self.session.query(Bookmark).delete()

    def test_create_bookmark(self):
        bookmark = Bookmark(title="Test Bookmark", url="http://test.com", 
                            notes="This is a test bookmark", date_added=date_object, date_edited=date_object)
        self.session.add(bookmark)
        self.session.commit()

        result = self.session.query(Bookmark).filter_by(title="Test Bookmark").first()
